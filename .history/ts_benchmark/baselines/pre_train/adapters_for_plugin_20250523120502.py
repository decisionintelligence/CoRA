...
from ts_benchmark.plugin.plugin_timer import Plugin as Plugin_Timer
...
class PluginConfig:
    def __init__(self, **kwargs):
        for key, value in DEFAULT_PlUGIN_HYPER_PARAMS.items():
            setattr(self, key, value)
...
class PluginAdapter(ModelBase):
    def __init__(self, model_name, model_class, **kwargs):
        super(PluginAdapter, self).__init__()
    ...
    def forecast_fit(
        ...
        pth = path + '/' + self.model_name + '_' + self.config.dataset + str(self.config.seq_len)  + '_' + str(self.config.pred_len) + '.pth'
        if self.model_name == "TimerModel":
            self.model = Plugin_Timer(backbone=self.model, pth=pth, configs=self.config)
        else:
            self.model = Plugin(backbone=self.model, pth=pth, configs=self.config)
        print(
            "---------------------------------------------------------- CoRA + ",
            self.model_name,
        )
...
def generate_model_factory(
    ...
    def model_factory(**kwargs) -> PluginAdapter:
        """
        ...
        """
        return PluginAdapter(model_name, model_class, **kwargs)
...
def Plugin_adapter(model_info: Type[object]) -> object:
