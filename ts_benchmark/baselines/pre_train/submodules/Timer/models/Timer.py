...
from .TimerBackbone import Model as TimerBackbone_Model
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.backbone = TimerBackbone_Model(configs)
        # Decoder
    ...
    def forcast_for_plugin_decoder(self,x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
    ...
    def forcast_for_plugin(self,x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
    ...
    def forcast_for_plugin_in(self, embedding):
        ...
        B, M, _, _ = embedding.shape
        B = B * M
        M = 1
        dec_in = rearrange(embedding, 'b k p d -> (b k) p d')
        # Encoder
        dec_out, attns = self.decoder(dec_in) # [B * M, N, D]
        ...
        # De-Normalization from Non-stationary Transformer
        dec_out = dec_out * self.stdev + self.means

        return dec_out

    def forcast_for_plugin_out(self,x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        
        self.B, L, self.M = x_enc.shape
    ...
    def denorm_for_plugin(self, forcast):
        
        return forcast * self.stdev + self.means
