...
def build_model_config(args: argparse.Namespace, config_data: Dict) -> Dict:
    ...
    if args.plugin_hyper_params is not None:
        args.plugin_hyper_params = [
            None if item == "None" else item for item in args.plugin_hyper_params
        ]
        if len(args.model_name) > len(args.plugin_hyper_params):
            args.plugin_hyper_params.extend(
                [None] * (len(args.model_name) - len(args.plugin_hyper_params))
            )
    else:
        args.plugin_hyper_params = [None] * len(args.model_name)

    for adapter, model_name, model_hyper_params, plugin_hyper_params in zip(
        args.adapter, args.model_name, args.model_hyper_params, args.plugin_hyper_params
    ):
        model_config["models"].append(
            {
                ...
                "plugin_hyper_params": json.loads(plugin_hyper_params)
                if plugin_hyper_params is not None
                else {},
            }
        )
...
if __name__ == "__main__":
    ...
    if args.report_method == "csv":
        ...
        # model_hyper_params = json.loads(args.model_hyper_params[0])
        # filename = f"_num_before_{model_hyper_params['num_before']}_num_after_{model_hyper_params['num_after']}_plugin_dim_{model_hyper_params['plugin_dim']}.csv"
