...
class MOMENT(nn.Module):
    ...
    def forcast_for_plugin(self,x_enc, input_mask: torch.Tensor = None,
        mask: torch.Tensor = None,device=None):
        if input_mask is None:
        ...
        return TimeseriesOutputs(input_mask=input_mask, forecast=dec_out), enc_out
    
    def denorm_for_plugin(self, input):
        input = input.permute(0,2,1)
        # [batch_size x n_channels x forecast_horizon]
        return self.normalizer(x=input, mode="denorm").permute(0,2,1)
